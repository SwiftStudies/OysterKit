...
internal enum STLRTokens : Int, TokenType, CaseIterable, Equatable {
    ...
    var rule : Rule {
        switch self {
            ...
            /// rule
            case .rule:
                return [    T.lhs.rule,    T.whitespace.rule.require(.zeroOrMore),    [T.expression.rule].sequence.annotatedWith([RuleAnnotation.error:RuleAnnotationValue.string("Expected expression"),RuleAnnotation.custom(label:"fatal"):RuleAnnotationValue.set]),    T.whitespace.rule.require(.zeroOrMore)].sequence.reference(.structural(token: self))
...
public struct STLR : Codable {
    ...
    public enum Terminal : Codable {
        ...
        enum CodingKeys : Swift.String, CodingKey {
            case characterSet,endOfFile,terminalString,regex,characterRange
        }
        ...
        public init(from decoder: Decoder) throws {
            ...
            throw DecodingError.valueNotFound(Expression.self, DecodingError.Context(codingPath: decoder.codingPath, debugDescription: "Tried to decode one of CharacterSet,Swift.String,TerminalString,Swift.String,CharacterRange but found none of those types"))
        }
        public func encode(to encoder:Encoder) throws {
            var container = encoder.container(keyedBy: CodingKeys.self)
            switch self {
